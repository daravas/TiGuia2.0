...
class SignInWithAppleCoordinator: NSObject, ASAuthorizationControllerPresentationContextProviding{
    ...
    private func sha256(_ input: String) -> String {
        let inputData = Foundation.Data(input.utf8)
        ...
        if let appleIDCredential = authorization.credential as? ASAuthorizationAppleIDCredential {
            ...
            Auth.auth().currentUser?.link(with: credential, completion: {(authResult, error) in
                if let error = error, (error as NSError).code == AuthErrorCode.credentialAlreadyInUse.rawValue {
                    print("The user you are trying to sign in has already been linked")
                    if let updatedCredencial = (error as NSError).userInfo[AuthErrorUserInfoUpdatedCredentialKey] as? OAuthCredential{
                        print("Signing in using updated credencials")
                        Auth.auth().signIn(with: updatedCredencial) { (authResult, error) in
...
//          print("Erro ao tentar mudar o nome: \(error)")
        }
    }
